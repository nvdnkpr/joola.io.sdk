...
joolaio.visualisation.picker.Metrics.prototype.setSelected = function (sender, metricname, multiple) {
    ...
    if (metricname != '') {
        $(_this.container.find('.jbtn')[0]).html((metricname.length > 21 ? metricname.substring(0, 21) + '...' : metricname) + '<span class="jio-caret"></span>');
        if (!$(_this.container).hasClass('on')) {
            $(_this.container).addClass('on');
            if (_this.options.allowremove)
                $(_this.container).append('<button class="jio-close">&times;</button>');

            $($(this.container).find('.jio-close')).off('click');
            $($(this.container).find('.jio-close')).on('click', function (e) {
        ...
        var s = ' + add metric <span class="jio-caret"></span>';
        if (_this.options.type == 'button')
            s = _this.options.placeholdertext + '<span class="jio-caret"></span>';
...
joolaio.visualisation.picker.Metrics.prototype.disableMetric = function (sender, metricname) {
...
joolaio.visualisation.picker.Metrics.prototype.baseHTML = function (sender) {
    ...
    if (_this.options.type == 'button') {
        $html.append('<a class="jio-btn jbtn">' + _this.options.placeholdertext + '<span class="jio-caret"></span></a>');
    }
    else {
        if (_this.options.showgrip)
            $html.append('<div class="grip"></div>');
        $html.append('<div class="customadd metric jbtn"> + add metric <span class="jio-caret"></span></div>');
    }
