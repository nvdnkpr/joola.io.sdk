...
joolaio.provide('joolaio.visualisation.picker.DateBox');
 ...
 */
joolaio.visualisation.picker.DateBox.init = function (options, container) {
    var _this = this;
    ...
    this.min_date = new joolaio.objects.Query().SystemStartDate();
    this.max_date = new joolaio.objects.Query().SystemEndDate();
    ...
    this.cssPath = joolaio.contenthost + '/assets/css/datebox.css';
    //$("head").append('<style>' + new JAPI.Visualisation().CSS(this.cssPath) + '</style>');
    //$("head").append('<style type="text/css">@import "' + joolaio.contentPath + this.cssPath + '";</style> ');

    if (!joolaio.dateboxcssloaded) {
        joolaio.dateboxcssloaded = true;
    ...
    if (container)
        ...
        matchedContainers = $('.joolaio.picker.datebox');
    ...
    $(matchedContainers).each(function (index, item) {
        ...
        if (!$(this).parent().hasClass('prettyprint')) {
            ...
            joolaio.debug.log('INFO', 'joolaio.visualisation.picker.DateBox', 6, 'Applying to container (\'' + this.id + '\')');
    ...
    joolaio.debug.log('INFO', 'joolaio.visualisation.picker.DateBox', 5, '...init (' + executionTime + 'ms)');

};

joolaio.visualisation.picker.DateBox.addDays = function (o, days) {
// keep in mind, months in javascript are 0-11
    return new Date(o.getFullYear(), o.getMonth(), o.getDate() + days);
}

joolaio.visualisation.picker.DateBox.draw = function (Container) {
    var _this = this;
    ...
    $('.dates').append('<span class="datelabel fromdate">' + joolaio.date.formatDate(_this.base_fromdate) + '</span>');
    $('.dates').append(' - ');
    //$('.dates').append('<span class="datelabel todate">' + _this.formatDate(_this.base_todate) + '</span>');
    $('.dates').append('<span class="datelabel todate">' + joolaio.date.formatDate(_this.base_todate) + '</span>');
    $('.dates').append('<div class="compare" style="display:none">Compare to: <span class="datelabel compare fromdate">' + joolaio.date.formatDate(_this.compare_fromdate) + '</span> - <span class="datelabel compare todate">' + joolaio.date.formatDate(_this.compare_todate) + '</span></div>');
    ...
    $('.optionscontainer').append('' +
        '<hr class="divider">' +
        '<div class="_buttons"><button class="jio-btn apply" value="Apply">Apply</button>' +
    ...
    $('.datepicker').datepicker({
        ...
        onSelect: function (dateText, inst) {
            ...
            $('.joolaio .optionscontainer .selector').val('custom');
            ...
            switch (_this.currentMode) {
                case 'base-from':
                    ...
                    $($('.daterange.baserange .dateoption')[0]).val(joolaio.date.formatDate(_this.base_fromdate));
                    $($('.daterange.baserange .dateoption')[0]).removeClass('invalid');
                    $($('.daterange.baserange .dateoption')[1]).val(joolaio.date.formatDate(_this.base_fromdate));
                ...
                case 'base-to':
                    ...
                    $($('.daterange.baserange .dateoption')[1]).val(joolaio.date.formatDate(_this.base_todate));
                    $($('.daterange.baserange .dateoption')[1]).removeClass('invalid');
    ...
    $($('.daterange.baserange .dateoption')[0]).blur(function (e) {
        ...
        $('.jio-btn.apply').removeClass('disabled');
        $('.jio-btn.apply').prop('disabled', false);
        _this.currentMode = 'base-from';
        _this.handleChange();
    });
    ...
    $($('.daterange.baserange .dateoption')[0]).keyup(function (e) {
        if (new Date($(this).val()) == 'Invalid Date' || new Date($(this).val()) > _this.base_todate || new Date($(this).val()) > _this.max_date || new Date($(this).val()) < _this.min_date) {
            ...
            $('.jio-btn.apply').addClass('disabled');
            $('.jio-btn.apply').prop('disabled', true);
        }
        else {
            $(this).removeClass('invalid');
            $('.jio-btn.apply').removeClass('disabled');
    ...
    $('.joolaio .optionscontainer .cancel').click(function (e) {
        ...
        $('.joolaio.datebox .container').click();
    });
    $('.joolaio .optionscontainer .selector').change(function (e) {
        switch (this.value) {
            case 'today':
    ...
    /*
     ...
     my:"right bottom",
     at:"right top",
     of:$('.joolaio.datebox .container'), // or $("#otherdiv),
     offset:_this.offsetX +  ' ' + _this.offsetY
     });*/

    $('.joolaio.datebox .container').off('click');
    $('.joolaio.datebox .container').on('click', function (e) {
        ...
        if ($(this).hasClass('expanded')) {
            ...
            //$('.picker').offset($('.joolaio.datebox .container').offset());
            $('.joolaio.picker.datebox .picker').show();
            $('.joolaio.picker.datebox .picker').offset({top: $('.joolaio.picker.datebox .picker').offset().top, left: $('.joolaio.picker.datebox .container').offset().left - $('.joolaio.picker.datebox .picker').outerWidth() + $('.joolaio.picker.datebox .container').outerWidth()});
    ...
    $('.joolaio.datebox').click(function (e) {
        e.stopPropagation();
    });

    $('body').click(function (e) {
        $('.joolaio.datebox .container').removeClass('expanded');
        $('.joolaio.datebox .picker').hide();
    });

    $('.joolaio.datebox .optionscontainer .apply').click(function (e) {
        $('.joolaio.datebox .container').removeClass('expanded');
        $('.joolaio.picker.datebox .picker').hide();
...
joolaio.visualisation.picker.DateBox.drawCell = function (date) {
...
joolaio.visualisation.picker.DateBox.fixDate = function (timestamp, zero) {
...
joolaio.visualisation.picker.DateBox.formatDate = function (date) {

    return joolaio.date.formatDate(date);
    ...
    return Highcharts.dateFormat('%b %e, %Y', joolaio.visualisation.picker.DateBox.fixDate(date));
}


joolaio.visualisation.picker.DateBox.handleChange = function (options) {
    var _this = this;
    ...
    if (this.isCompareChecked) {
        $('.joolaio .optionscontainer .checker').prop("checked", true);
        $('.joolaio .optionscontainer .daterange.comparerange').show();

    }
    else {
        $('.joolaio .optionscontainer .checker').prop("checked", false);
        $('.joolaio .optionscontainer .daterange.comparerange').hide();
    }

    $('.joolaio .optionscontainer .checker').off('click');
    $('.joolaio .optionscontainer .checker').on('click', function (e) {
...
joolaio.visualisation.picker.DateBox.updateLabels = function (sender, options) {
    /*
    ...
    $('.dates .datelabel.fromdate').text(joolaio.date.formatDate(options.base_fromdate));
    $('.dates .datelabel.todate').text(joolaio.date.formatDate(options.base_todate));
    ...
    $('.dates .datelabel.compare.fromdate').text(joolaio.date.formatDate(options.compare_fromdate));
    $('.dates .datelabel.compare.todate').text(joolaio.date.formatDate(options.compare_todate));

    if (options.compare) {

        $(".joolaio.picker.datebox").addClass('compare');
        $(".joolaio.picker.datebox .picker").addClass('compare');
        $('.joolaio.picker.datebox .dates .compare').show();
        //$('.picker').css({'margin':'30px;','backgroundColor':'red'});
    }
    else {
        $(".joolaio.picker.datebox").removeClass('compare');
        $(".joolaio.picker.datebox .picker").removeClass('compare');
        $('.joolaio.picker.datebox .dates .compare').hide();
    }


}

joolaio.visualisation.picker.DateBox.DateUpdate = function () {
    var _this = this;
...
joolaio.visualisation.picker.DateBox.setState = function (sender) {
    var _this = sender;
    joolaio.state.fromdate = _this.base_fromdate;
    joolaio.state.todate = _this.base_todate;
    joolaio.state.comparePeriod = _this.comparePeriod;
    joolaio.state.compare_fromdate = _this.compare_fromdate;
    joolaio.state.compare_todate = _this.compare_todate;

    //joolaio.saveState('Date change');
};

joolaio.visualisation.picker.DateBox.getState = function (sender) {
    var _this = sender;


    //return;

    if (joolaio.state.fromdate != null) {
        _this.base_fromdate = new Date(joolaio.state.fromdate);
        _this.base_todate = new Date(joolaio.state.todate);
        _this.comparePeriod = joolaio.state.comparePeriod;
        _this.compare_fromdate = new Date(joolaio.state.compare_fromdate);
        _this.compare_todate = new Date(joolaio.state.compare_todate);
    }
};

joolaio.visualisation.picker.DateBox.registerDateUpdate = function (callback, sender) {
    this.callbacks.push({callback: callback, sender: sender});
};

joolaio.visualisation.picker.DateBox.getDate = function () {
    var _this = this;
    var options = new Object();
...
joolaio.visualisation.picker.DateBox.setDate = function (sender, fromdate, todate) {
    var _this = sender;
...
joolaio.debug.log('INFO', 'joolaio.visualisation.picker.DateBox', 6, 'JS source loaded');
